        ...
        <div class="bg-[#111] border border-white/5 rounded-xl overflow-hidden shadow-2xl">
            <div class="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <label class="cursor-pointer bg-gold text-dark px-4 py-2 rounded text-[10px] uppercase tracking-widest font-bold hover:bg-gold-light transition-all flex items-center gap-2">
                    ...
                    <tr v-for="customer in filteredCustomers" :key="customer.id" class="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                ...
                                <button @click="deleteCustomer(customer)" class="p-2 text-gray-500 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
...
const handleFileUpload = async (event) => {
    ...
    try {
        ...
        const response = await fetch('/admin/clients/import', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': token,
        ...
        if (!response.ok) {
            ...
            throw new Error(data.message || 'Error importing clients.')
        }
...
const deleteCustomer = async (customer) => {
    if (!confirm(`Are you sure you want to delete the client ${customer.name}?`)) return;
    
    try {
        const token = document.head.querySelector('meta[name="csrf-token"]').content;
        
        const response = await fetch(`/admin/clients/${customer.id}`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': token,
                'Accept': 'application/json'
